...
"""
    직접 만든 클래스
"""
class CustomDataset(ClusteringDataset):
    def __init__(self, data, labels=None, num_clusters=None):
        super().__init__(data, labels, num_clusters)
    ...
    def setup(cls, filepath_samples: str, filepath_labels: str = None, num_clusters: int = None):
        """
        :param filepath_samples: 데이터셋 CSV 파일 경로
        :param filepath_labels: 라벨 경로 -> 없음
        """
        # 데이터셋 CSV 파일 불러오기
        X = pd.read_csv(filepath_samples)

        # 결측치 열 제외
        cols_na = X.columns[X.isnull().sum() > 0].tolist() 
        X = X.drop(columns=cols_na)
